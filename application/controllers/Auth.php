...
class Auth extends CI_Controller
{
    ...
    private function _login()
    {
        ...
        if ($user) {
            ...
            if ($user['aktif'] == 1) {
                ...
                $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">
                Email yang anda masukan tidak aktif</div>');
                redirect('auth');
            }
        } else {
            $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">
            Email yang anda belum terdaftar!</div>');
            redirect('auth');
        }
    }
    ...
    public function registration()
    {
        ...
        $this->form_validation->set_rules('nama', 'nama', 'required|trim'); //trim digunakan agar penulisan spasi di awal/akhir tidak dibaca
        ...
        if ($this->form_validation->run() == false) {
            ...
            Akun anda berhasil dibuat. Silahkan aktivasi akun anda</div>');
            redirect('auth');
        }
    }

    public function _kirimEmail($token, $type)
    {
        $config = [
            'protocol' => 'smtp', //simple main transfer protocol
            'smtp_host' => 'ssl://smtp.googlemail.com',
            'smtp_user' => 'proyek2batik@gmail.com',
            'smtp_pass' => 'batikmulya123',
            'smtp_port' => 465,
            'mailtype' => 'html',
            'charset' => 'utf-8',
            'newline' => "\r\n"
        ...
        $this->email->from('proyek2batik@gmail.com', 'Batik Mulya'); //email sumber
        ...
        if ($type == 'verify') {
            ...
            $this->email->message('Klik link ini untuk memverifikasi akun anda : 
                <a href="' . base_url() . 'auth/verify?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '">Aktivasi</a>'); //isi email
        } else if ($type == 'forgot') {
            $this->email->subject('Reset Password'); //subjek email
            $this->email->message('Klik link ini untuk mereset password anda : 
                <a href="' . base_url() . 'auth/resetpassword?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '">Reset Password</a>'); //isi email
        }
    ...
    public function lupa_password()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $email = $this->input->post('email');
            $user = $this->db->get_where('users', ['email' => $email, 'aktif' => 1])->row_array();
            ...
            if ($user) {
                ...
                $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
                Periksa email untuk mereset password anda!</div>');
                redirect('auth/lupa_password');
            } else {
                $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">
                Email belum terdaftar atau diaktivasi!</div>');
                redirect('auth/lupa_password');
            }
        }
    }
